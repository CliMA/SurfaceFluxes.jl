...
function Psi(uf::Cheng, ζ, tt::HeatTransport)
    ...
    if abs(ζ) < eps(typeof(uf.L))
        ...
        if ζ >= 0
            ...
            @error(
                "Volume-averaged form of Cheng (2005) heat SCF for ζ >> 0 not yet implemented. Defaulting to Nishizawa (2018) formulation.",
                maxlog = 1
            )
        end
...
function Psi(uf::Cheng, ζ, tt::MomentumTransport)
    ...
    if abs(ζ) < eps(typeof(uf.L))
        ...
        if ζ >= 0
            ...
            @error(
                "Volume-averaged form of Cheng (2005) momentum SCF for ζ >> 0 not yet implemented. Defaulting to Nishizawa (2018) formulation.",
                maxlog = 1
            )
        end
...
function psi(uf::Beljaars, ζ, tt::HeatTransport)
    ...
    if 0 < ζ
        ...
        power_term = -1 * (1 + 2 * _a_h * ζ / 3)^(FT(3 / 2))
...
universal_func_type(::Type{T}) where {T <: BeljaarsParams} = BeljaarsType()
