...
TD.cp_m(tp::MockThermoParams, phase::TD.PhasePartition) = tp.cp_d
TD.cp_m(tp::MockThermoParams, q_tot, q_liq, q_ice) = tp.cp_d
TD.cv_m(tp::MockThermoParams, phase::TD.PhasePartition) = tp.cp_d - tp.R_d
TD.gas_constant_air(tp::MockThermoParams, phase::TD.PhasePartition) = tp.R_d
...
SF.Δz(inputs::MockInputs) = inputs.z_int - inputs.z_sfc
...
@testset "Variance Functions" begin
    ...
    var_u = SF.u_variance(param_set, inputs, ustar, L_MO)
    ...
    var_theta = SF.scalar_variance(param_set, inputs, theta_star, L_MO)
    ...
    var_u_stable = SF.u_variance(param_set, inputs, ustar, L_MO_stable)
    @test isapprox(var_u_stable, (ustar * sqrt(3.75))^2; rtol=1e-4)
    
    # Heat: Constant 2.0
    var_theta_stable = SF.scalar_variance(param_set, inputs, theta_star, L_MO_stable)
    ...
    var_u_neutral = SF.u_variance(param_set, inputs, ustar, L_MO_neutral)
    ...
    var_theta_wrapper = SF.theta_variance(param_set, inputs, shf, ustar, L_MO)
    var_theta_explicit = SF.scalar_variance(param_set, inputs, theta_star_calc, L_MO)
