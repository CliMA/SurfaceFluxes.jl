...
@testset "Direct Bulk Flux Formulas" begin
    ...
    @testset "Latent Heat Flux (LHF) Signs" begin
        ...
        E = SF.evaporation(
            thermo_params, inputs, g_h, q_tot_int, q_vap_sfc, ρ_sfc,
        )
        @test E > 0
        ...
        # is allowed by the function signature, but typically inputs.q_tot_int matches q_tot_int.
        ...
        E_cond = SF.evaporation(
            thermo_params, inputs, g_h, q_tot_int, q_sfc_cond, ρ_sfc,
        )
        @test E_cond < 0
    ...
    @testset "Buoyancy Flux Signs" begin
        ...
        buoy_flux = SF.buoyancy_flux(
            param_set, thermo_params, shf, lhf, T_sfc, q_vap_sfc, FT(0), FT(0), ρ_sfc,
        )
        @test buoy_flux > 0
        ...
        buoy_flux_neg = SF.buoyancy_flux(
            param_set, thermo_params, shf_neg, lhf, T_sfc, q_vap_sfc, FT(0), FT(0), ρ_sfc,
        )
